...
test('input field',async({page})=>{
    ...
    await usingTheGridEmailInput.fill('test@gmaail.com')
...
for (const color in colors) {
  ...
  if (color !== "Corporate") {
    await dropDownMenu.click(); // reopen only if needed
  }
}
})
    ...
    test('Web Table',async({page})=>{
        ...
        await page.locator('input-editor').getByPlaceholder('E-mail').fill('test@testing1.com')
        await page.locator('.nb-checkmark').click()
        await expect(targetRowByID.locator('td').nth(5)).toHaveText('test@testing1.com')
      ...
      test('datepicker',async({page})=>{
        ...
        let data = new Date()
        data.setDate(data.getDate()+27) // Here you acn seleect the date
        ...
        let calenderMonthhAndYear = await page.locator('nb-calendar-view-mode').textContent()
        const expectedMonthAndYear = `${expectedMonthLong} ${expectedYear}`
        while(!calenderMonthhAndYear?.includes(expectedMonthAndYear)){
            await page.locator('nb-calendar-pageable-navigation [data-name="chevron-right"]').click()
            calenderMonthhAndYear = await page.locator('nb-calendar-view-mode').textContent()
